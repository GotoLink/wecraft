...
public class BlockDrill extends Block {
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par6, float par7, float par8, float par9) {
		...
		if (drillStatus == 0) {
			...
			world.scheduleBlockUpdate(i, j, k, this.blockID, this.tickRate(world) * 2);
		} else {
			int y = j;
			while (y > 3) {
				if (world.getBlockId(i, y - 1, k) == Wecraft.drillHead.blockID) {
					y--;
				} else {
					break;
				}
			...
			if (y < j) {
				...
				world.scheduleBlockUpdate(i, y, k, Wecraft.drillHead.blockID, this.tickRate(world));
			} else {
	...
	public void updateTick(World world, int i, int j, int k, Random random) {
		if (world.getBlockMetadata(i, j, k) != 0) {
			...
			for (int l = j - 1; l >= 4; l--) {
				...
				if (i1 != headID && i1 != Block.bedrock.blockID) {
					...
					break;
				} else if (i1 == Block.bedrock.blockID) {
					if (world.getBlockId(i, l + 1, k) == headID) {
